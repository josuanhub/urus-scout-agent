...
// ─── AgentVerse link strategy ─────────────────────────────────────────────────
// Solo se incluye el link en ~10% de los replies donde el tema es
// trust / reputation / identity / scoring. No rompe el personaje de Scout.
// Se activa cuando:
//   1. El comentario toca esos temas
//   2. Math.random() < 0.10
const AGENTVERSE_URL = process.env.AGENTVERSE_URL || "https://agentverse-pi.vercel.app";
const URUSVERIFY_URL = process.env.URUSVERIFY_URL || "https://www.urusverify.com";


const TRUST_KEYWORDS = [
  "trust", "reputation", "score", "identity", "verify", "credential",
  "signal", "ranking", "leaderboard", "governance", "continuity",
  "confianza", "reputación", "identidad", "puntaje"
];

function commentMentionsTrustTopic(body) {
  const lower = String(body || "").toLowerCase();
  return TRUST_KEYWORDS.some(kw => lower.includes(kw));
}

function shouldInjectAgentVerseLink(commentBody) {
  if (!commentMentionsTrustTopic(commentBody)) return false;
  return Math.random() < 0.80; // 80% de probabilidad
}

// El link se añade como una línea final sutil, en tono Scout (no spam)
function buildAgentVerseSuffix() {
  const phrases = [
  `\n\nThe live signal layer is public: ${AGENTVERSE_URL}`,
  `\n\nBehavior patterns are already being tracked: ${AGENTVERSE_URL}`,
  `\n\nReputation infrastructure: ${AGENTVERSE_URL}`,
  `\n\nSee who actually performs: ${AGENTVERSE_URL}`,
  `\n\nVerify any agent's trust score: ${AGENTVERSE_URL}`,
  `\n\nAgent certification is live: ${URUSVERIFY_URL}`,
  `\n\nMost agents generate activity. Few generate signal: ${URUSVERIFY_URL}`,
  `\n\nTrust infrastructure for AI agents: ${URUSVERIFY_URL}`,
];
  return phrases[Math.floor(Math.random() * phrases.length)];
}
// ─────────────────────────────────────────────────────────────────────────────

// ─── Circuit breaker de suspensión ───────────────────────────────────────────
// Si Moltbook devuelve 403 "suspended until <ISO>", bloqueamos el loop
// hasta que pase ese tiempo — sin acumular más ofensas.
let suspendedUntil = null; // Date | null

function parseSuspensionDate(errMessage) {
  const match = String(errMessage || "").match(/suspended until ([0-9T:.Z+-]+)/i);
...
function isSuspended() {
  ...
  if (Date.now() < suspendedUntil.getTime()) return true;
  suspendedUntil = null; // ya pasó — limpiar
  return false;
}

function isMoltbookSuspensionError(err) {
  const msg = String(err?.message || "");
  const status = Number(err?.statusCode || err?.status || 0);
  return (status === 403 || msg.includes("403")) && msg.toLowerCase().includes("suspended");
}

function handleMoltbookError(err, context) {
  if (isMoltbookSuspensionError(err)) {
    const until = parseSuspensionDate(err.message);
    if (until) {
      suspendedUntil = until;
      console.warn(`SCOUT_SUSPENDED_UNTIL ${until.toISOString()} [${context}]`);
    } else {
      suspendedUntil = new Date(Date.now() + 2 * 60 * 60 * 1000); // back-off 2h
      console.warn(`SCOUT_SUSPENDED_UNKNOWN_UNTIL (back-off 2h) [${context}]`);
    }
    throw err;
  }
  console.error(`MOLTBOOK_ERROR [${context}]`, err?.message || err);
}
// ─────────────────────────────────────────────────────────────────────────────
...
async function publishIfNeeded(output) {
  ...
  let postResult;
  try {
    postResult = await createPost({
      title,
      content: finalPublishText,
      submolt_name: "general"
    });
  } catch (err) {
    handleMoltbookError(err, "createPost");
    return null;
  }
...
// ─── Deduplicación robusta de replies (FIX del ban por duplicate_comment) ────
// Doble nivel:
//   1. replyHistory en memoria por commentId (original)
//   2. scout_memory por hash(postId + body normalizado) — robusto a IDs inconsistentes
function buildReplyDedupeKey(postId, commentBody) {
  const normalized = normalizeText(commentBody).slice(0, 200);
  return `reply_dedupe:${hashKey(`${postId}:${normalized}`)}`;
}

async function alreadyRepliedTo(postId, commentId, commentBody, replyHistory) {
  // Nivel 1: por commentId en historial
  if (replyHistory.find(x => x.commentId === commentId)) return true;
  // Nivel 2: por hash en scout_memory (robusto a ID inconsistente)
  const dedupeKey = buildReplyDedupeKey(postId, commentBody);
  const exists = await getScoutMemoryByKey(dedupeKey);
  return Boolean(exists);
}

async function markReplied(postId, commentBody) {
  const dedupeKey = buildReplyDedupeKey(postId, commentBody);
  await upsertScoutMemory({
    memoryKey: dedupeKey,
    kind: "reply_dedupe",
...
async function extractRecentUsefulComments(feed) {
  ...
  for (const post of items.slice(0, 50)) {
    ...
    for (const c of comments) {
      ...
      // commentId robusto: si no viene, genera hash de postId+author+body
      const rawId = String(c?.id || "").trim();
      const commentId = rawId || hashKey(`${postId}:${author}:${body.slice(0, 80)}`);
...
async function maybeReplyToComments(feed) {
  ...
  console.log("SCOUT_REPLY_CANDIDATES", candidates.length);
  ...
  for (const c of candidates) {
    ...
    // Dedup robusto (2 niveles — fix del ban)
    const alreadyDone = await alreadyRepliedTo(c.postId, c.commentId, c.body, replyHistory);
...
Return valid JSON.`;
    ...
    if (!replyText) {
      ...
      continue; // nada que postear → saltar (fix: antes continuaba sin el continue)
    }

    // ── Estrategia 10% AgentVerse link ──────────────────────────────────────
    // Solo si el comentario toca temas de trust/reputation/identity
    // y cae en el 10% aleatorio — se añade al final del reply, en tono Scout
    let finalReplyText = replyText;
    if (shouldInjectAgentVerseLink(c.body)) {
      finalReplyText = replyText + buildAgentVerseSuffix();
      console.log("AGENTVERSE_LINK_INJECTED", { author: c.author, postId: c.postId });
    }
    // ────────────────────────────────────────────────────────────────────────

    // Scoring desde interacción
    ...
    try {
      await createComment({
        post_id: c.postId,
        content: finalReplyText   // usa el texto con o sin link según estrategia
      });

      // Marcar como respondido en AMBOS niveles de dedup
      await markReplied(c.postId, c.body);
      ...
      // Si es suspensión, cortar el ciclo completo
      if (isMoltbookSuspensionError(err)) {
        handleMoltbookError(err, "createComment");
        return;
      }
...
// ─── Score agents from feed directly ─────────────────────────────────────────
// Guarda scores de todos los agentes del feed en cada ciclo,
// independientemente de si el scout les responde o no.
async function scoreFeedAgents(feed) {
  const items = Array.isArray(feed?.items)
    ? feed.items
    : Array.isArray(feed?.posts)
      ? feed.posts
  ...
  for (const post of items.slice(0, 50)) {
    ...
    if (!author || author === "unknown" || author.includes("urus-scout")) continue;

    const commentCount = Number(post?.comment_count || post?.commentCount || 0);
    const likeCount    = Number(post?.like_count    || post?.likeCount    || 0);
    const content      = String(post?.content || post?.body || "").trim();

    // Score heurístico basado en actividad real del post
    const utility     = Math.min(10, 4 + Math.floor(commentCount / 5));
    const trust       = 5;
    const clarity     = content.length > 100 ? 6 : 4;
    const momentum    = Math.min(10, 4 + Math.floor((commentCount + likeCount) / 10));
    const originality = 5;
    const risk        = 3;
    const scout_score = utility + trust + clarity + momentum + originality - risk;

    await upsertScoutMemory({
      memoryKey: `agent_score:${author}:${Date.now()}`,
      kind: "agent_score",
      payload: {
        author,
        score: { utility, trust, clarity, momentum, originality, risk, scout_score },
        classification: scout_score >= 30 ? "HIGH_SIGNAL" : scout_score >= 20 ? "MID_SIGNAL" : "NOISE",
        source: "feed_scan",
        ts: new Date().toISOString()
      }
    });
  ...
  console.log("FEED_AGENTS_SCORED", saved);
}
// ─────────────────────────────────────────────────────────────────────────────

async function runOneCycle() {
  try {
    // ── Circuit breaker: respetar suspensión sin acumular ofensas ──
    if (isSuspended()) {
      console.log("SCOUT_LOOP_SUSPENDED_SKIPPING", {
        resumesAt: suspendedUntil?.toISOString()
      });
      return;
    }
    ...
    await scoreFeedAgents(replyFeed);
  } catch (err) {
    // Si era suspensión ya fue logueada — solo salir limpio
    if (isMoltbookSuspensionError(err)) return;
