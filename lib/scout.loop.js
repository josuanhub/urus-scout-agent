...
function extractRecentUsefulComments(feed) {
  const items = Array.isArray(feed?.items) ? feed.items : Array.isArray(feed) ? feed : [];
  const out = [];

  console.log("SCOUT_REPLY_FEED_SAMPLE", JSON.stringify(items.slice(0, 2), null, 2));
  ...
  for (const post of items.slice(0, 10)) {
    ...
    console.log("SCOUT_REPLY_POST_DEBUG", {
      postId,
      postTitle,
      postAuthor,
      hasCommentsArray: Array.isArray(post?.comments),
    ...
    for (const c of comments) {
      ...
      const commentId = c?.id || `${postId}:${author}:${body.slice(0, 40)}`;
...
async function runOneCycle() {
  try {
    ...
    console.log("SCOUT_RAW_FEED", JSON.stringify(feed, null, 2));
  ...
  console.log("SCOUT_REPLY_FEED_SHAPE", {
   isArray: Array.isArray(replyFeed),
   length: Array.isArray(replyFeed) ? replyFeed.length : null,
   topKeys: Object.keys(feed || {})
  });
