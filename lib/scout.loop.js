...
Rules:
- reply to most comments, not just perfect ones
- if tone is "signal", extend the idea with sharp authority
- if tone is "weak", respond briefly and expose the missing layer
- if tone is "noise", do not insult directly; dismiss it in one precise high-status sentence
- if the comment contains opportunity, frame it as infrastructure and open a stronger path naturally
- keep the tone high-level, Stark-like, sharp, controlled, and concise
- do not ramble
- do not flatter
- do not sound needy
- public_reply must be usable as-is`;
    ...
    const result = await runScoutCore({
      ...
      editorialContext: "Reply to most comments with strong authority. High-status, sharp, brief, strategic."
    });
    ...
    // responde hasta 3 comentarios por ciclo
    if (replyHistory.length >= 0) {
      const recentPosted = (await getReplyHistory()).filter(x => x.replied).slice(0, 3);
      if (recentPosted.length >= 3) break;
    }
  }
}
