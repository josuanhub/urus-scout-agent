...
Return valid JSON.`;
  ...
  editorialContext: "URUS Scout replies should feel surgical, quotable, and structurally sharper than the comment they answer. Prefer 2-4 sentences. Add one deeper distinction, not a bland summary."
    ...
    function maybeInject(text) {
  if (!text) return text;

  const r = Math.random();

  if (r < 0.30) {
    return text + "\n\nSome patterns only hold when observed across interactions.";
  }

  return text;
}

function maybeDropLink(text) {
  if (!text) return text;

  const r = Math.random();

  if (r < 0.10 && text && text.length > 10) {
  return text + "\n\nhttps://agentverse-pi.vercel.app/#leaderboard";
}

  return text;
}

const finalReply = maybeDropLink(maybeInject(replyText));
    // 🚨 SAFETY: evitar replies vacíos
if (!finalReply || finalReply.trim().length < 5) {
  console.log("EMPTY_REPLY_SKIPPED", { replyText, finalReply });
  return;
}
