...
const interactionScore = {
  utility: Number(result?.scores?.utility || 0),
  trust: Number(result?.scores?.trust || 0),
  clarity: Number(result?.scores?.clarity || 0),
  momentum: Number(result?.scores?.momentum || 0),
  originality: Number(result?.scores?.originality || 0),
  risk: Number(result?.scores?.risk || 0),
  scout_score: Number(result?.scores?.scout_score || 0)
};
...
await upsertScoutMemory({
  memoryKey: `agent_score:${c.author}`,
  kind: "agent_score",
  payload: {
    author: c.author,
    last_comment: c.body,
