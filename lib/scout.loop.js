...
async function extractRecentUsefulComments(feed) {
  ...
  for (const post of items.slice(0, 50)) {
    ...
    // TEMP: permitir todos los posts
// if (!postAuthor.includes("urus-scout")) continue;

    // FORZAR lectura aunque diga 0 comments
const commentCount = Number(post?.comment_count || post?.commentCount || 0);
// if (commentCount <= 0) continue;

    let fullPost;
    try {
      fullPost = await getPostById(postId);
    } catch (err) {
      console.error("SCOUT_GET_POST_ERROR", postId, err?.message || err);
      continue;
    }
    ...
    const comments = Array.isArray(detailedPost?.comments) ? detailedPost.comments : [];
    const postTitle = String(detailedPost?.title || post?.title || "Untitled").trim();

    console.log("POST_COMMENTS_FOUND", postId, comments.length);
...
async function extractCommentsFromOwnTrackedPosts() {
  ...
  for (const postId of postIds.slice(0, 50)) {
    ...
    if (postId === "28021f93-653c-4816-b540-39637dbaf1e6") {
  console.log("DEBUG_SINGLE_POST", JSON.stringify(detailedPost, null, 2));
}
...
Task:
Produce a public URUS Scout reply.

Rules:
- reply briefly
- be sharp, clear, high-status
- do not ramble
- do not flatter
- if the comment is thoughtful, add one strong insight
- if the comment is weak, answer shortly and move on
- public_reply must be usable as-is`;

    const result = await runScoutCore({
      message,
      mode: "reply",
      targetFormat: "Comment",
      editorialContext: "Reply to comments under URUS Scout posts. Short, sharp, strategic."
    });
...
const isOpportunity = await checkMoltbookCommentOpportunity(c);

if (isOpportunity) {
  const actionReply = await generateActionReply(c);
...
async function checkMoltbookCommentOpportunity(comment) {
...
async function generateActionReply(comment) {
  return `You're thinking in the right direction.

Most people try to patch this manually.
The real leverage is building a system around it.`;
}
